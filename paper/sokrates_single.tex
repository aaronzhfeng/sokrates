...
External symbolic solvers can provide exactly this missing signal: cheap, dense, step-level labels that distinguish valid inferences from invalid ones.
...
% === BEGIN INPUT: paper/figures/architecture_v2.tex ===
...
% === END INPUT: paper/figures/architecture_v2.tex ===
...
% === BEGIN INPUT: paper/tables/options.tex ===
% tables/options.tex
...
\label{tab:options}
\end{table}
% === END INPUT: paper/tables/options.tex ===
...
% === BEGIN INPUT: paper/figures/trace_example.tex ===
...
\end{figure}
% === END INPUT: paper/figures/trace_example.tex ===
...
If a step cannot be parsed for verification, we accept it as valid (a limitation of the current parser, which is designed for PrOntoQA's restricted language fragment).
For FOLIO, we rely on the provided FOL annotations and a rule-level verifier: we implement checks for a subset of option types and mark unsupported rules as \textsc{Unknown} (counted as invalid in our step/trace metrics).
When available, we configure Z3-based entailment/consistency checks with a 5\,s timeout.
Because Actions are grammar-constrained, failures are dominated by incorrect index selection, misuse of rules, or unsupported rule types rather than malformed syntax.
...
% === BEGIN INPUT: paper/algorithms/oak_loop.tex ===
...
% === END INPUT: paper/algorithms/oak_loop.tex ===
...
% === BEGIN INPUT: paper/tables/hyperparams.tex ===
...
% === END INPUT: paper/tables/hyperparams.tex ===
...
% === BEGIN INPUT: paper/tables/main_results.tex ===
...
\end{table}
% === END INPUT: paper/tables/main_results.tex ===
...
The \sokrates{} loop is data-efficient: with only $K{=}2$ samples/problem for preference learning, we achieve a $33\times$ improvement in trace validity.
...
% === BEGIN INPUT: paper/tables/transfer.tex ===
% tables/transfer.tex
...
% === END INPUT: paper/tables/transfer.tex ===
...
% === BEGIN INPUT: paper/tables/ablations.tex ===
...
\label{tab:ablations}
\end{table}
% === END INPUT: paper/tables/ablations.tex ===
...
% === BEGIN INPUT: paper/figures/full_prompt.tex ===
...
% === END INPUT: paper/figures/full_prompt.tex ===
