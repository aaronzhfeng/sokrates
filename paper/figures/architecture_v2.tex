...
\begin{tikzpicture}[
    ...
    % --- 6. Solver ---
    \node[solver, below=1.6cm of build] (z3) {FOL Solver (Z3)};
    ...
    \begin{scope}[on background layer]
        ...
        \node[
            ...
            inner sep=0.5cm,
            label={[anchor=south, yshift=0.2cm]north:\textbf{\textit{Micro OaK Loop} (repeat $N=2$ iterations)}}
        ] (loop_box) {};
    \end{scope}
...
\end{tikzpicture}
\caption{The \sokrates{} architecture. Data flows through an optionizer to create Thought/Action training pairs for SFT, producing initial policy $\pi_0$. The Micro OaK Loop iteratively generates traces, verifies them with a FOL solver, updates the option-success predictor $\qhat$, builds preferences from valid/invalid traces, and applies DPO to align the policy. Dashed arrows show feedback loops within the iterative training.}
